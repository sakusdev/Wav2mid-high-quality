...
function makeFixtureWav(path) {
  ...
  for (let i = 0; i < totalSamples; i += 1) {
    ...
    for (const [start, end, pitches] of events) {
      ...
      const local = t - start;
      const envelope = Math.min(1, local / 0.015) * Math.min(1, (end - t) / 0.08) * (0.8 + 0.2 * Math.exp(-1.2 * local));
      for (const midi of pitches) {
        const f = 440 * 2 ** ((midi - 69) / 12);
        x += envelope * (Math.sin(2 * Math.PI * f * t) + 0.35 * Math.sin(4 * Math.PI * f * t) + 0.15 * Math.sin(6 * Math.PI * f * t)) / pitches.length;
      }
    }
    buf.writeInt16LE(Math.max(-32767, Math.min(32767, Math.round(x * 0.28 * 32767))), 44 + i * 2);
  }
